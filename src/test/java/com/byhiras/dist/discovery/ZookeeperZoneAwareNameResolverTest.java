...
public class ZookeeperZoneAwareNameResolverTest {
    ...
    public void shouldResolveSingleAddress() throws Exception {
        ...
        ServiceDiscovery serviceDiscovery = new ServiceDiscovery(zkHost);
        serviceDiscovery.removeServiceRegistry("pingPongService");
        serviceDiscovery.registerService("pingPongService", URI.create("dns://localhost:" + serverPort));
    ...
    public void shouldResolveMultipleAddressesAndAllowLoadBalancing() throws Exception {
        ...
        ServiceDiscovery serviceDiscovery = new ServiceDiscovery(zkHost);
        serviceDiscovery.removeServiceRegistry("pingPongService");
        serviceDiscovery.registerService("pingPongService", URI.create("dns://localhost:" + server1Port));
        serviceDiscovery.registerService("pingPongService", URI.create("dns://localhost:" + server2Port));

        ManagedChannel channel =
                ManagedChannelBuilder.forTarget("zk://pingPongService")
    ...
    public void shouldResolveMultipleAddressesAndProvideUpdatesWhenOneDisconnects() throws Exception {
        ...
        serviceDiscovery.deregister("pingPongService", URI.create("dns://localhost:" + server2Port));
        server1Count.set(0);
        server2Count.set(0);
